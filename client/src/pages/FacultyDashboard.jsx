...
const FacultyDashboard = () => {
    ...
    return (
        <div className="p-4 w-full space-y-6 min-w-0 bg-bg-main dark:bg-[#020617] text-text-main transition-colors duration-300">
            {/* Header & Main Actions */}
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-black text-[var(--text-main)] tracking-tight mb-2">SCHEDULE</h1>
                    <div className="flex items-center gap-2 text-[var(--text-muted)]">
                        <Clock size={12} className="text-[var(--brand-primary)]" />
                        <span className="text-[10px] font-black uppercase tracking-[0.1em]">{currentTime.toLocaleTimeString()}</span>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="p-3 bg-bg-card text-text-muted rounded-xl border border-border-main shadow-sm hover:bg-brand-500/10 transition-all relative"
                        >
                            <Bell size={18} />
                            <div className="absolute top-2 right-2 w-2 h-2 bg-rose-500 border-2 border-[var(--bg-card)] rounded-full animate-bounce"></div>
                        </button>
                        {showNotifications && (
                            <div className="absolute right-0 mt-4 w-[300px] sm:w-96 bg-[#0F172A] rounded-2xl shadow-2xl p-4 z-[100] border border-slate-800 animate-in fade-in slide-in-from-top-4 duration-300">
                                <div className="flex items-center justify-between mb-4">
                                    <h4 className="text-white font-black text-xs uppercase tracking-widest">Upcoming</h4>
                                    <span className="px-2 py-1 bg-bg-card/10 text-[#3B82F6] text-[8px] font-black rounded-lg">LIVE SYNC</span>
                                </div>
                                <div className="space-y-3 max-h-[300px] overflow-y-auto scrollbar-hide">
                                    {timetable.filter(t => getStatusInfo(t.day, t.slot).text === 'Upcoming').slice(0, 3).map((notif, idx) => (
                                        <div key={idx} className="p-3 bg-bg-card/5 rounded-xl border border-white/5 hover:bg-bg-card/10 transition-all group">
                                            <div className="flex items-start justify-between mb-1">
                                                <span className="text-[9px] font-black text-[#10B981] uppercase tracking-widest">{notif.day}</span>
                                                <span className="text-[9px] font-black text-slate-500 uppercase">{notif.slot}</span>
                                            </div>
                                            <h5 className="text-white text-xs font-bold mb-1 line-clamp-1">{notif.subject?.name}</h5>
                                            <div className="flex items-center justify-between">
                                                <span className="text-[9px] font-black text-text-muted uppercase tracking-widest">Room {notif.classroom?.roomNumber}</span>
                                                <span className="text-[9px] font-bold text-[#3B82F6]">{notif.batch?.name}</span>
                                            </div>
                                        </div>
                                    ))}
                                    {timetable.filter(t => getStatusInfo(t.day, t.slot).text === 'Upcoming').length === 0 && (
                                        <div className="py-6 text-center opacity-30 italic text-white text-[10px] uppercase tracking-widest font-black">No upcoming classes</div>
                                    )}
                                </div>
                            </div>
                        )}
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="p-3 bg-brand-500 text-white rounded-xl shadow-lg shadow-brand-500/20 hover:bg-brand-600 transition-all"
                        >
                            <Plus size={18} />
                        </button>
                        {isActionMenuOpen && (
                            <div className="absolute right-0 mt-2 w-48 bg-[var(--bg-card)] rounded-2xl shadow-xl border border-[var(--border-main)] py-2 z-[100] animate-in fade-in slide-in-from-top-2">
                                <div className="px-4 py-2 text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)]">Actions</div>
                                <button onClick={() => { setShowAddClassModal(true); setIsActionMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-brand-500/10 text-left text-xs font-bold text-[var(--text-main)]">
                                    <Plus size={14} className="text-brand-500" /> Add Class
                                </button>
                                <button onClick={() => { setShowUploadModal(true); setIsActionMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-indigo-500/10 text-left text-xs font-bold text-[var(--text-main)]">
                                    <Database size={14} className="text-indigo-500" /> Upload Data
                                </button>
                                <div className="h-px bg-[var(--border-main)] my-1"></div>
                                <button onClick={() => { exportToExcel(); setIsActionMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-emerald-500/10 text-left text-xs font-bold text-[var(--text-main)]">
                                    <FileSpreadsheet size={14} className="text-emerald-500" /> Export Excel
                                </button>
                                <button onClick={() => { exportToPDF(); setIsActionMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-rose-500/10 text-left text-xs font-bold text-[var(--text-main)]">
                                    <FileText size={14} className="text-rose-500" /> Download PDF
                                </button>
                                <div className="h-px bg-[var(--border-main)] my-1"></div>
                                <button onClick={() => { fetchResources(); setIsActionMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-500/10 text-left text-xs font-bold text-[var(--text-main)]">
                                    <RotateCcw size={14} className="text-slate-500" /> Refresh Data
                                </button>
                            </div>
                        )}
                    </div>
            ...
            <div className="flex flex-col gap-3">
                <div className="flex gap-3">
                    ...
                    <div className="relative flex-1">
                        <select
                            ...
                            className="w-full pl-4 pr-8 py-3.5 bg-[var(--bg-card)] border border-[var(--border-main)] rounded-xl text-[10px] font-black uppercase tracking-widest text-[var(--text-main)] focus:outline-none focus:ring-2 focus:ring-brand-500/20 shadow-sm appearance-none cursor-pointer"
                        >
                    ...
                    <div className="relative flex-1">
                        <select
                            ...
                            onChange={(e) => {
                                ...
                                if (e.target.value !== 'ALL') setSearchDate('');
                            }}
                            className="w-full pl-4 pr-8 py-3.5 bg-[var(--bg-card)] border border-[var(--border-main)] rounded-xl text-[10px] font-black uppercase tracking-widest text-[var(--text-main)] focus:outline-none focus:ring-2 focus:ring-brand-500/20 shadow-sm appearance-none cursor-pointer"
                        >
                            <option value="ALL">ALL DAYS</option>
                            {DAYS.filter(d => d !== 'Saturday').map(day => (
                                <option key={day} value={day}>{day.slice(0, 3).toUpperCase()}</option>
                            ))}
                        </select>
                        <ChevronRight className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 rotate-90 pointer-events-none" />
                    </div>
                ...
                <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-main)] shadow-sm p-3 flex flex-col gap-3">
                    {activeTab === 'free' ? (
                        <div className="flex gap-3">
                            <select
                                value={selectedBlock}
                                onChange={(e) => setSelectedBlock(e.target.value)}
                                className="flex-1 p-3 bg-[var(--bg-main)] border border-[var(--border-main)] rounded-lg text-xs font-bold text-[var(--text-main)] focus:outline-none"
                            >
                                {uniqueBlocks.map(block => (
                                    <option key={block} value={block}>Bldg {block}</option>
                                ))}
                            </select>
                            <select
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            {activeTab === 'faculty' ? (
                                <select
                                    ...
                                    className="w-full p-3 bg-[var(--bg-main)] border border-[var(--border-main)] rounded-lg text-xs font-bold text-[var(--text-main)] focus:outline-none"
                                >
                                ...
                                <div className="flex gap-2">
                                    <div className="relative flex-1">
                                        ...
                                        <input
                                            ...
                                            placeholder={activeTab === 'room' ? 'Search rooms...' : 'Search batch...'}
                                            className="w-full pl-10 pr-4 py-3 bg-[var(--bg-main)] border border-[var(--border-main)] rounded-lg text-xs font-bold text-[var(--text-main)] focus:outline-none"
                                        />
                                    </div>
                                    <button
                                        onClick={handleSearch}
                                        className="px-4 py-3 bg-[#3B82F6] text-white rounded-lg font-black text-[10px] uppercase tracking-widest shadow-md hover:bg-blue-600"
                                    >
                                        GO
                                    </button>
                                </div>
            ...
            <div className="flex items-center gap-3 overflow-x-auto pb-2 scrollbar-hide">
                <div className="flex-none bg-brand-50 dark:bg-brand-500/10 p-3 rounded-xl min-w-[110px] border border-brand-100 dark:border-brand-500/20">
                    <span className="text-[9px] font-black uppercase tracking-widest text-brand-500">Lectures</span>
                    <h4 className="text-xl font-black text-[var(--text-main)] mt-1">{timetable.filter(t => selectedDay ? t.day === selectedDay : true).length}</h4>
                </div>
                <div className="flex-none bg-emerald-50 dark:bg-emerald-500/10 p-3 rounded-xl min-w-[110px] border border-emerald-100 dark:border-emerald-500/20">
                    <span className="text-[9px] font-black uppercase tracking-widest text-emerald-500">Ongoing</span>
                    <h4 className="text-xl font-black text-[var(--text-main)] mt-1">{timetable.filter(t => (selectedDay ? t.day === selectedDay : true) && getStatusInfo(t.day, t.slot).text === 'Ongoing').length}</h4>
                </div>
                <div className="flex-none bg-amber-50 dark:bg-amber-500/10 p-3 rounded-xl min-w-[110px] border border-amber-100 dark:border-amber-500/20">
                    <span className="text-[9px] font-black uppercase tracking-widest text-amber-500">Total Hrs</span>
                    <h4 className="text-xl font-black text-[var(--text-main)] mt-1">{timetable.filter(t => selectedDay ? t.day === selectedDay : true).length}</h4>
                </div>
            </div>
