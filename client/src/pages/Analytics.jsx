...
const Analytics = () => {
    ...
    const StatCard = ({ title, value, icon: Icon, color }) => (
        <div className="bg-bg-card p-4 rounded-3xl shadow-sm border border-[var(--border-main)] flex flex-col items-center justify-center text-center gap-2">
            <div className={`p-3 rounded-2xl ${color} bg-opacity-10`}>
                <Icon size={20} className={color.replace('bg-', 'text-')} />
            </div>
            <div>
                <h3 className="text-xl font-black text-[var(--text-main)] leading-tight">{value || 0}</h3>
                <p className="text-[var(--text-muted)] text-[9px] font-bold uppercase tracking-widest mt-0.5">{title}</p>
            </div>
        </div>
    );

    return (
        <div className="p-4 pb-32 space-y-6 bg-bg-main min-h-screen text-text-main transition-colors duration-300 overflow-x-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div>
                    <h1 className="text-2xl font-black text-[var(--text-main)]">Analytics <span className="text-[var(--brand-primary)]">Engine</span></h1>
                ...
                <div className="flex items-center p-1 bg-bg-card rounded-2xl border border-border-main w-full sm:w-auto self-start">
                    <button onClick={() => setViewMode('personal')} className={`flex-1 sm:px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${viewMode === 'personal' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'text-slate-400 hover:text-slate-600'}`}>Personal</button>
                    <button onClick={() => setViewMode('global')} className={`flex-1 sm:px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${viewMode === 'global' ? 'bg-[var(--brand-primary)] text-white shadow-lg shadow-brand-500/30' : 'text-slate-400 hover:text-slate-600'}`}>Global</button>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-6">
                <div className="bg-bg-card p-5 sm:p-8 rounded-3xl shadow-sm border border-[var(--border-main)] relative overflow-hidden group">
